...
def get_domestic_eurex_option_master_dataframe(base_dir):
    ...
    urllib.request.urlretrieve("https://new.real.download.dws.co.kr/common/master/fo_eurex_code.mst.zip", os.path.join(base_dir, "fo_eurex_code.mst.zip"))
    os.chdir(base_dir)

    fo_eurex_code_zip = zipfile.ZipFile('fo_eurex_code.mst.zip')
    fo_eurex_code_zip.extractall()
    fo_eurex_code_zip.close()
    file_name = os.path.join(base_dir, "fo_eurex_code.mst")
    
    # df1 : '상품종류','단축코드','표준코드','한글종목명'
    tmp_fil1 = os.path.join(base_dir, "fo_eurex_code_part1.tmp")
    tmp_fil2 = os.path.join(base_dir, "fo_eurex_code_part2.tmp")
    ...
    tmp_fil3 = os.path.join(base_dir, "fo_eurex_code_part3.tmp")
    ...
    DF.to_csv('fo_eurex_code.csv',index=False)  # 현재 위치에 엑셀파일로 저장

    # zip 파일과 압축 해제한 파일 모두 삭제
    os.remove(os.path.join(base_dir, "fo_eurex_code.mst.zip"))
    os.remove(os.path.join(base_dir, "fo_eurex_code.mst"))

    # 임시 파일 삭제
    os.remove(os.path.join(base_dir, "fo_eurex_code_part1.tmp"))
    os.remove(os.path.join(base_dir, "fo_eurex_code_part2.tmp"))
    os.remove(os.path.join(base_dir, "fo_eurex_code_part3.tmp"))
